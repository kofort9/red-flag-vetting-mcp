...
function parseString(args: Record<string, unknown> | undefined, key: string): string {
  const val = args?.[key];
...
function parseOptionalNumber(args: Record<string, unknown> | undefined, key: string): number | undefined {
  const val = args?.[key];
  if (val === undefined || val === null) return undefined;
  if (typeof val === 'number') return val;
  if (typeof val === 'string') {
    if (val.trim() === '') throw new Error(`Invalid ${key}: empty string is not a number`);
    const parsed = Number(val);
    if (Number.isNaN(parsed)) throw new Error(`Invalid ${key}: "${val}" is not a number`);
    return parsed;
  }
...
function parseOptionalEnum<T extends string>(
  ...
  if (typeof val !== 'string' || !allowed.includes(val as T)) {
    throw new Error(`Invalid ${key}: expected one of ${allowed.join(', ')}, got "${val}"`);
  }
  return val as T;
}
...
export async function startServer(): Promise<void> {
  ...
  if (!courtClient) {
    logInfo('CourtListener token not configured — court record checks disabled');
  }
        ...
        case 'refresh_data':
          return toMcpResponse(
            await tools.refreshData(store, {
              source: parseOptionalEnum(args, 'source', ['irs', 'ofac', 'all'] as const),
            })
          );
